...
Pi is conversational and benefits from a concise project instruction directory. Load only the skills that match the current task. Never load all private memory, and prefer summaries before full originals.
...
Use a light-touch protocol: configuration, conventions summary, current context, and one relevant skill. The configured Praxis allocation is **3K** for a **lightweight** context window. Follow `token-optimizer.md` before loading optional context.
