...
Identify and reduce security risks in code, configuration, and memory.

## When to Activate

- Authentication, secrets, trust boundaries, and untrusted input.
- The task explicitly requests this capability.
- A related agent or plan identifies this capability as a dependency.

## When NOT to Activate

- Purely visual wording with no security impact.
...
Memory writes are denied until the candidate content has passed `memory/security/deny-list.md` and the procedure in `memory/security/scan-procedure.md`.

1. Scan frontmatter, body, links, and code examples with every deny-list pattern.
2. Reject the write and warn the user when a critical or warning pattern matches.
3. Check the classification and route sensitive non-secret content to `memory/private/`.
4. Log the category, action, file, timestamp, and agent name without copying the secret.
5. Write only clean, classified content and mark the security scan as passed.
