...
Cursor uses the root `.cursorrules` file and project-level Chat context. Load only the skills that match the current task. Never load all private memory, and prefer summaries before full originals.
...
Keep `.cursorrules` concise and synchronized with semantic memory; load only the relevant skill for the active task. The configured Praxis allocation is **8K** for a **128K** context window. Follow `token-optimizer.md` before loading optional context.
