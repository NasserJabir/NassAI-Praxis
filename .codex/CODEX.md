...
Codex CLI uses the `.codex/` project directory and chat-based task execution. Load only the skills that match the current task. Never load all private memory, and prefer summaries before full originals.
...
Use the same full startup and post-task protocol as Claude Code, with an 8K Praxis allocation inside the 128K model context. The configured Praxis allocation is **8K** for a **128K** context window. Follow `token-optimizer.md` before loading optional context.
