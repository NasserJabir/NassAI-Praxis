...
Kimi supports Memory Instructions and project file uploads. Load only the skills that match the current task. Never load all private memory, and prefer summaries before full originals.
...
Kimi may load full semantic memory and two or three relevant skills, while still keeping private memory excluded. The configured Praxis allocation is **10K** for a **200K+** context window. Follow `token-optimizer.md` before loading optional context.
