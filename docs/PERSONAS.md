...
A **Persona** is a Markdown definition of how a task should be approached. It records thinking style, priorities, risk tolerance, recurring review questions, and communication preferences. Personas make those characteristics explicit and reviewable instead of leaving them as an implicit prompt habit.
...
| Learned proposals | Allows a repeated, evidenced reasoning improvement to be considered. | Does not update the canonical Persona automatically. |
...
```text
Observed session evidence → Persona proposal → Human review → Approved Markdown change
```
