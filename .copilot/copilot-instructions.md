...
GitHub Copilot receives repository and file-level instructions through its instruction files. Load only the skills that match the current task. Never load all private memory, and prefer summaries before full originals.
...
Use the conventions summary rather than loading the full semantic file; load a skill when opening a matching file type. The configured Praxis allocation is **2K** for a **8K** context window. Follow `token-optimizer.md` before loading optional context.
