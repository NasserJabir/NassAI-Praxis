...
Windsurf separates Cascade agent work from Edit inline work. Load only the skills that match the current task. Never load all private memory, and prefer summaries before full originals.
...
Cascade receives the standard 8K budget inside approximately 128K; Edit should use only the concise rules mirror. The configured Praxis allocation is **8K** for a **128K** context window. Follow `token-optimizer.md` before loading optional context.
