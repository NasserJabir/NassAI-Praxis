...
Gemini CLI uses `.gemini/GEMINI.md` and has a context window of approximately 1M+ tokens. Load only the skills that match the current task. Never load all private memory, and prefer summaries before full originals.
...
Gemini may load all semantic memory and relevant skills, and full episodic memory for complex debugging, but lazy loading remains recommended for efficiency. The configured Praxis allocation is **15K** for a **1M+** context window. Follow `token-optimizer.md` before loading optional context.
